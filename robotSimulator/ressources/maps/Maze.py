            ...
            for j in range(self._nbColumn+1):
                ...
                if random.randint(0,1):
                    if j!=0:
                        ...
                        item=Object(Representation(Line(dh, self.DEFAULT_BORDER_SCREEN_WIDTH, self.DEFAULT_BORDER_SCREEN_COLOR)))
                        self._environment.addObject(item,j*self.INTERVAL_SIZE,i*self.INTERVAL_SIZE)
                else:
                    if i!=0:
                        dw=self._width%self.INTERVAL_SIZE if j==self._nbColumn else self.INTERVAL_SIZE
                        item=Object(Representation(Line(dw, self.DEFAULT_BORDER_SCREEN_WIDTH, self.DEFAULT_BORDER_SCREEN_COLOR)))
                        self._environment.addObject(item,j*self.INTERVAL_SIZE,i*self.INTERVAL_SIZE,-90)
                self._mazeElements.append(item)
